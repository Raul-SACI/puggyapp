...
export function IncomesAnalysis({ incomes, overrides, inflation, reload }: Props) {
  ...
  const data = useMemo(() => {
    ...
    // Categorías del mes, fuentes y medios de cobro
    ...
    for (const o of curOcc) {
      ...
      const src = o.source || 'Sin fuente'
      srcMap.set(src, (srcMap.get(src) ?? 0) + o.amount)
    ...
    const sources: CatRow[] = [...srcMap.entries()]
      .map(([name, amount]) => ({ name, amount }))
      .sort((a, b) => b.amount - a.amount)
    const srcMaxVal = Math.max(0, ...sources.map((c) => c.amount))
  ...
  return (
    <div className="screen">
      ...
      {/* Ingresos por fuente */}
      <div className="card chart-card">
        <h3 className="chart-title">Ingresos por fuente ({monthName})</h3>
        {data.sources.length === 0 ? (
          <p className="muted">No hay ingresos este mes en esta moneda.</p>
        ) : (
          <div className="hbars">
            {data.sources.map((c) => {
              const pct = data.srcMaxVal > 0 ? (c.amount / data.srcMaxVal) * 100 : 0
              return (
                <div className="hbar-row" key={c.name}>
                  <div className="hbar-head">
                    <span className="hbar-name">{c.name}</span>
